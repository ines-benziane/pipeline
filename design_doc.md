...
- **Q1 — Stockage et bases de données ?**  
  ...
  **Ajout (2026-08-17, notes de réunion avec PY) :** piste évoquée d'un module dédié — schéma `DICOM > METHOD > BDD > REPORT` — qui reprendrait/généraliserait ce que `data_reader`/`data_writer` font déjà dans `medical_report`, extrait pour être réutilisable par n'importe quelle méthode (standard ou custom), avec pour destinations possibles : le JSON attendu par `medical_report`, un CSV/Excel agrégé sur une cohorte, ou une future vraie BDD. Lié à l'idée (différée, non prioritaire tant que le chemin heureux `dixon3pt` n'est pas fini) de permettre aux chercheurs de brancher leurs propres méthodes tant qu'elles respectent les contrats : une méthode custom dont les stats ne suivent pas le contrat `get_result → medical_report` perd la compatibilité avec le report standard (attendu, pas un problème à résoudre), mais reste utilisable pour des exports cohorte custom. Frontière encore floue (« je n'ai pas compris la frontière » — ses mots) entre ce qui reste dans `get_result` (par méthode) et ce qui irait dans ce module partagé (la destination/le formatage de sortie) — voir aussi la discussion en session sur `methods/get_results/getresults.py`.
